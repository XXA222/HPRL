...
class DataProvider:
    def __init__(
        ...
        optimization = self._config.get("hedge_optimization_runtime", {})
        optimization = optimization if isinstance(optimization, dict) else {}
        memory = optimization.get("memory", {})
        memory = memory if isinstance(memory, dict) else {}
        cache_mode = str(memory.get("backtesting_cache_mode", "official")).strip().lower()
        if cache_mode not in {"official", "bounded", "consume"}:
            raise OperationalException(
                "hedge_optimization_runtime.memory.backtesting_cache_mode must be "
                "official/bounded/consume"
            )
        cache_entries = memory.get("backtesting_cache_max_entries", 1)
        if not isinstance(cache_entries, int) or isinstance(cache_entries, bool) or cache_entries < 1:
            raise OperationalException(
                "hedge_optimization_runtime.memory.backtesting_cache_max_entries must be >= 1"
            )
        self.__backtesting_cache_mode = cache_mode
        self.__backtesting_cache_max_entries = int(cache_entries)
    ...
    def historic_ohlcv(self, pair: str, timeframe: str, candle_type: str = "") -> DataFrame:
        ...
        # Official Freqtrade keeps historical frames cached and returns a copy.
        # That is ideal for Hyperopt reuse, but a one-shot compact Hedge backtest
        # can opt into consume mode: the caller owns the loaded frame directly and
        # no duplicate cached copy remains resident.
        if self.__backtesting_cache_mode == "consume":
            return load_history()

        if saved_pair not in self.__cached_pairs_backtesting:
            self.__cached_pairs_backtesting[saved_pair] = load_history()
            if self.__backtesting_cache_mode == "bounded":
                while len(self.__cached_pairs_backtesting) > self.__backtesting_cache_max_entries:
                    oldest = next(iter(self.__cached_pairs_backtesting))
                    if oldest == saved_pair and len(self.__cached_pairs_backtesting) == 1:
                        break
                    self.__cached_pairs_backtesting.pop(oldest, None)
    ...
    def clear_cache(
        ...
        *,
        ...
        """Clear runtime dataframe/message caches.

        The default preserves Freqtrade's historical-backtest cache because Hyperopt
        intentionally reuses it across epochs.  One-shot consumers that have already
        detached the data they need (for example compact Hedge replay) can request a
        full release explicitly.
        """
        self.__cached_pairs.clear()
        self.__slice_index.clear()
        if backtesting:
            self.__cached_pairs_backtesting.clear()
            self.__slice_date = None
        if external:
            self.__producer_pairs_df.clear()
            self.__producer_pairs.clear()
        if messages:
